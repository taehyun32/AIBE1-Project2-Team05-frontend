...
async function initializeMyMatchesView(nickname, elements) {
  ...
  async function loadAndRenderData(page = 0) {
    ...
    try {
      ...
      displayMessage(
        contentListElement,
        error.message || '매칭 정보를 불러오는 중 오류가 발생했습니다.',
        'error'
      );
    }
  }
    ...
    matches.forEach((match, index) => {
      ...
      const buttonsHtml = !isInProgress
        ...
        : `<div class="flex items-center gap-2">
            ...
            mentorData.mentorId
          }" data-session-id="${match.sessionId}" data-contact-link="${
            mentorData.contactLink || ''
          }">
               연락하기
             </button>
      ...
      matchElement.innerHTML = `
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-primary text-sm font-medium mt-1 truncate" title="${
              mentorData.interest || ''
            }">${mentorData.interest || '주요 분야 없음'}</p>
  ...
  function setupActionButtons() {
    contentListElement
      .querySelectorAll('button[id^="contact-btn-"]')
      .forEach((button) => {
    ...
    contentListElement
      .querySelectorAll('button[id^="terminate-btn-"]')
      .forEach((button) => {
        button.addEventListener('click', function () {
  ...
  async function handleMatchTerminate(mentorId, sessionId) {
    if (confirm('정말 이 매칭을 종료하시겠습니까?')) {
      try {
        // API 호출: 매칭 종료 (API 엔드포인트 확인 필요)
        // 예시: /api/v1/matching/sessions/${sessionId}/terminate 또는 /api/v1/matches/${sessionId} (DELETE)
        const apiUrl = `/api/v1/matching/sessions/${sessionId}/terminate`; // 실제 API 경로로 수정
        const response = await fetch(apiUrl, {
          method: 'POST', // 또는 'PUT', 'DELETE' 등 API 명세에 따름
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include'
          // body: JSON.stringify({ mentorId: mentorId }) // 필요시 body 추가
        });
        ...
        const result = await response.json().catch(() => ({})); // 빈 객체로 폴백

        if (
          response.ok &&
          (result.code === 'SUCCESS' || result.status === 200)
        ) {
          alert('매칭이 종료되었습니다.');
          loadAndRenderData(currentPage); // 현재 페이지 데이터 다시 로드
        } else {
          throw new Error(
            result.message || `매칭 종료 실패 (상태: ${response.status})`
          );
        }
      } catch (error) {
