    ...
    matches.forEach((match) => {
      ...
      const statusClass =
        status === "진행중"
          ...
          : "bg-blue-100 dark:bg-blue-700 text-blue-700 dark:text-blue-100 text-xs px-2 py-1 rounded-full whitespace-nowrap";
      // mentor 정보 추출
      const {nickname, profileImageUrl, introduction, interest, activityTime, contactLink} = match.mentor;
...
function populateTalentList(talents) {
  ...
  if (talents && talents.length > 0) {
    ...
    ${talent.tags
      .map((tag) => `<span class="bg-gray-100 text-gray-700 text-sm px-3 py-1 rounded-full">${tag}</span>`)
      .join("")}
                </div>
